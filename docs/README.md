...
| **3** | [Architecture Reference](en_US/ArchitectureReference.md) | Pipeline design, ring buffer, audit chain, security model, backpressure, and wire boundaries. For core developers and systems engineers. |
| [KV and SIF Serialization Boundary](en_US/guides/KvWireAndSifMigration.md) | KV is the Record field model; SIF is the serialization boundary. |
...
| **3** | [架构参考手册](zh_CN/ArchitectureReference.md) | 管道设计、环形缓冲区、审计链、安全模型与线格式边界 |
| [KV 与 SIF 序列化边界](zh_CN/guides/KvWireAndSifMigration.md) | KV 是 Record 字段模型，SIF 是序列化边界 |
