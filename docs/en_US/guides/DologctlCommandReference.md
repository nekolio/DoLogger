...
Generate synthetic SIF test records for pipeline integration testing. The
output is length-prefixed SIF and can be replayed by the same decoder.
...
| Option | Description |
|:-:|:-:|
| `-f, --output-file <file>` | Output length-prefixed SIF file path |
...
Replay length-prefixed SIF records through the pipeline.
...
(The input may also be an older length-prefixed SIF file.)
