...
| Ring 3 字段完整性 | 纳入记录 `content_hash`；CRC32C 仅作为独立兼容校验。 |
...
| Ring 3 | 不可信扩展 | 任何插件（含 Red） | 所有插件 | `content_hash` |
...
- 纳入记录 `content_hash`；CRC32C 不是安全边界。
- 被**排除**在 Ed25519 签名覆盖范围之外。
- 可能被配置为不信任 `ext.*` 字段的 Filter 插件静默丢弃。
- 不携带任何 `audit_tags` 条目。

**理由**：Red 插件是零信任的。其输出参与记录内容哈希，但不纳入可选审计签名。需要不可否认性的系统必须启用审计路径和签名策略。
...
3. **从不**：堆地址或其他存储布局细节；只将溢出 payload 纳入规范序列化
...
| 层 | 机制 | 性能开销 | 保护范围 |
|:-:|:-:|:-:|:-:|
| KV 溢出字段 | `content_hash` 规范 payload 覆盖 | 可忽略 | 运行时篡改检测；CRC32C 仅作兼容校验 |
