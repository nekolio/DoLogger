...
| `fs.aio-max-nr` | 65536 | **262144** | 如果对文件接收器写入使用 AIO（规划中）。 |
...
| 槽数 | 约占用内存（256 B/记录） | 适用场景 |
|:-:|:-:|:-:|
| 65536（64K） | ~16 MB | 开发、仅审计（5K rec/s）——**默认值** |
| 131072（128K） | ~17 MB | 轻度生产（50K rec/s） |
| 262144（256K） | ~64 MB | 中等生产（100K-250K rec/s） |
...
```toml
[dologger]
ring_buffer_size = 65536        # 默认 64K 槽
```
...
|:-:|:-:|:-:|
| 环形缓冲区槽 | `ring_buffer_size * sizeof(record)` | 262144 x 256 B = **64 MB** |
| 对象池 | `ring_buffer_size * sizeof(record)` | 262144 x 256 B = **64 MB** |
...
| 250K rec/s（生产） | 262144 | 262144 | ~152 MB | 2 GB |
