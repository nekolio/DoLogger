...
`sink_shm` 通过有边界的跨进程共享内存环形缓冲区，将带长度前缀的 SIF 记录投递给外部消费进程。它与 `[sinks.*]` **分开接线**，不属于 sink 注册表。通过顶层 `[shm]` 表启用：
...
启用 `[watcher]` 时（见上文热重载小节），热重载会自动触发。显式启动控制面后，也可以通过重载端点排队请求：

```bash
# 请求会进入队列；Engine 所有者在下一次调用
# `poll_config_reload()` 时应用，且必须存在可读取的配置源路径。
# curl -X POST http://127.0.0.1:9090/reload
```

### 安全注意事项

- 控制面默认关闭，必须显式启用；在传输层认证实现前只允许绑定回环地址。
