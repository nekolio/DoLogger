...
消息 key 是契约；翻译只是展示层结果，不能参与分支判断、签名输入、WORM 内容、KVF1
规范字节或审计验证。
...
- `Record.message` 是不可变来源的原始字节，载荷类型必须明确：已验证 UTF-8、
  二进制/未知，或显式解码文本。
- KVF1、WORM 信封、签名和 content hash 使用原始字节及载荷类型，不做展示转码。
...
| E1 | 日志/接收器规范编码器与解码器接线 | KVF1 原始消息路径已建立；新字节 ABI TODO(author) |
...
| L4 | 编码/本地化转换基准与 fuzz 测试集 | 原始 KV 与 AUTO 回归测试已建立；完整套件 TODO(author) |
