...
const zh = {
  ...
  'panel-title': '全部平台 · 校验和 · 版本', 'panel-checksums': '校验和', 'panel-bench': '跑分数据',
  'panel-filter-title': '筛选资产', 'filter-assets': '个资产',
  'filter-release-type': '发布类型', 'filter-stable': '正式版',
  'filter-versions': '版本', 'filter-versions-hint': '最新两个版本',
  'filter-os': '系统', 'filter-arch': '架构', 'filter-kind': '类型',
  ...
  'sec-chain-detail': '每条审计记录装配时签名，按 LSN + prev_hash 链式关联，无法重排或伪造',
  'sinks-detail': '核心 → 11 种输出 sink 扇出，全部可插拔',
  'arch-detail': '完整架构参考图（装配 → 处理 → 格式化 → 扇出）',
  'rel-detail-rule': '命名规则：dologctl-<平台>-<架构>（v0.2.0 起附带版本前缀）',
...
const en: Record<string, string> = {
  ...
  'panel-title': 'All platforms · checksums · versions', 'panel-checksums': 'Checksums', 'panel-bench': 'Benchmarks',
  'panel-filter-title': 'Filter assets', 'filter-assets': 'assets',
  'filter-release-type': 'Release type', 'filter-stable': 'Stable',
  'filter-versions': 'Versions', 'filter-versions-hint': 'Latest 2 versions',
  'filter-os': 'OS', 'filter-arch': 'Arch', 'filter-kind': 'Type',
  ...
  'sec-chain-detail': 'Each audit record signed at assembly, chained by LSN + prev_hash — impossible to reorder or forge',
  'sinks-detail': 'Core fan-out to all 11 sinks — every one pluggable',
  'arch-detail': 'Full architecture reference diagram (assemble → process → format → fan-out)',
  'rel-detail-rule': 'Naming rule: dologctl-<os>-<arch> (versioned prefix from v0.2.0)',
