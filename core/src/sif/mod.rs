...
//! FlatBuffers bindings live at `core/src/sif/dologger_sif_generated.rs`
//! and are regenerated by `core/build.rs` when `flatc` is available
//! (committed so builds without `flatc` still work). See
//! [`encoder`] and [`decoder`] for Record↔SIF conversion.
//!
//! # Wire Format
//!
...
// Pull in the code generated by `core/build.rs` (flatc --rust). The generated
// code lives in a private module so lint allowances apply only to it, keeping
// the crate's clippy `-D warnings` gate clean.
//
// # Safety
//
// The generated code uses `unsafe` for raw pointer arithmetic (FlatBuffers
// wire format). It has been reviewed against the SAST audit baseline.
#[allow(
    missing_docs,
    dead_code,
    unused,
...
// The generated bindings stay crate-private. The public SIF API is the frame
// operations below (`encode_record` / `decode_record` / `validate_frame`), so
// the FlatBuffers internals can evolve without leaking into callers. Zero-copy
// field access for sinks will be added deliberately alongside the M4 fan-out.
...
pub use decoder::{decode_record, SifError};
