...
fn error_report_keeps_machine_key_separate_from_detail() {
    let report = ErrorReport::new(
        DO_LOG_ERR_SIF_INVALID,
        ErrorContext::new(ErrorOrigin::Serialization, "sif.decode").with_detail("offset=44"),
    );
    assert_eq!(report.key(), "sif.invalid");
    assert_eq!(report.code, DO_LOG_ERR_SIF_INVALID);
    assert!(report.diagnostic_message().contains("offset=44"));
    assert_eq!(report.fallback_message(), "SIF frame invalid");
}
...
fn descriptors_are_stable_for_wire_codes() {
    assert_eq!(error_descriptor(DO_LOG_ERR_SIF_INVALID).key, "sif.invalid");
    assert_eq!(
        error_descriptor(DO_LOG_ERR_SIF_INVALID).code,
        DO_LOG_ERR_SIF_INVALID
    );
}
