...
版本号格式：`MAJOR.MINOR.PATCH`（例如 `1.4.2`）

| 级别 | 含义 |
|:-:|:-:|
| **MAJOR** | 破坏性变更 -- ABI 版本号提升、VTable 布局变更、移除已废弃的符号、需要重新编译插件 |
| **MINOR** | 向后兼容的新功能、新插件类型、新的 VTable 函数（在末尾追加）、新的 C ABI 符号 |
| **PATCH** | 向后兼容的错误修复、安全补丁 |
...
| 发布 | 阶段 | 操作 |
|:-:|:-:|:-:|
| N（MINOR） | 标记为废弃 | 头文件标注 `DO_LOG_DEPRECATED`、编译器警告启用、文档更新并附迁移路径 |
| N+1（MINOR） | 警告增强 | 首次使用时记录运行时警告（每个进程一次）、Sysmon 发出 `DEPRECATED_API_USAGE` 事件、文档标记为"将在下一个 MAJOR 中移除" |
| N+2（MAJOR） | 移除 | 从头文件和库中移除符号、使用已移除符号的插件编译失败、引擎拒绝带有过时 ABI 版本的插件 |
...
以引擎 v2.0.0（ABI_VERSION = 2）为例：

| 插件信任级别 | 编译时 ABI 版本 | 结果 |
|:-:|:-:|:-:|
| Blue | ABI v1 | 加载被拒绝：`DO_LOG_ERR_PLUGIN_ABI` |
| Blue | ABI v2 | 加载接受，正常运行 |
| Yellow | ABI v1 | 加载被拒绝 |
| Yellow | ABI v2 | 加载接受 |
...
```
v1.0.0 发布                → 完全支持开始
v1.5.0（最终 MINOR）       → 完全支持继续
v2.0.0 发布                → v2 获得完全支持；v1 转为关键（6 个月窗口）
v2.0.0 + 6 个月            → v1 生命周期终止，不再支持
```
