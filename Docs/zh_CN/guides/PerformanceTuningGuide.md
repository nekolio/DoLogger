...
| 场景 | 推荐配置文件 | 理由 |
|:-:|:-:|:-:|
| 本地开发或 CI 测试 | `dev` | 快速启动，小占用，无需持久化 |
| 生产环境，需要法规合规（GDPR/HIPAA/PCI DSS） | `prod-audit` | Ed25519 签名、WORM 存储、fsync 持久性、强制执行不可降级的安全项 |
| 生产环境，最大吞吐量，无法规要求 | `prod-performance` | 最大环形缓冲区、最佳丢弃策略（below_warn）、Ed25519 签名可选 |
| 生产环境，通用，中等吞吐量需求 | `balanced` | 吞吐量和安全性的良好平衡 |
