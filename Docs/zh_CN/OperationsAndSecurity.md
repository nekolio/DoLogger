...
| 阶段 | 说明 |
|:-:|:-:|
| 阶段 1：启动轮换 | 生成新密钥对；旧密钥进入宽限期 |
| 阶段 2：宽限期（默认 7 天） | 两个密钥同时活跃；旧密钥签名进行中的记录；新密钥签名新提交的记录；验证器接受由任一密钥签名的记录 |
| 阶段 3：轮换完成 | 旧密钥被吊销（添加到 CRL）；所有新记录使用新密钥签名；旧密钥记录仍可用旧公钥验证 |
| 阶段 4：紧急吊销（可选） | 密钥指纹立即添加到 CRL；所有由被吊销密钥签名的记录验证失败 |
